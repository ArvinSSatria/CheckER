...
import { Info, DollarSign, FileText, ExternalLink } from "lucide-react";
...
export function ProfileCard({ username, followers, profilePicture, platform, postsAnalyzed, estimatedPostValue }: ProfileCardProps) {
  const [showTooltip, setShowTooltip] = useState(false);
  const isInstagram = platform === "instagram";

  const formula = isInstagram
    ? "((Avg Likes + Avg Comments) / Followers) × 100"
    : "((Avg Likes + Avg Comments + Avg Shares) / Avg Views) × 100";

  return (
    <Card className="w-full rounded-2xl border border-border bg-card soft-shadow transition-all duration-300 h-full">
      <CardContent className="p-8 sm:p-10 flex flex-col items-center justify-center text-center space-y-6 h-full">
        {/* Avatar */}
        <div className="relative">
          <div className="w-28 h-28 rounded-full overflow-hidden border-4 border-background bg-secondary flex items-center justify-center soft-shadow">
            {profilePicture ? (
              <img
                src={profilePicture}
        ...
        <div className="space-y-2">
          <a
            href={isInstagram ? `https://www.instagram.com/${username.replace("@", "")}/` : `https://www.tiktok.com/@${username.replace("@", "")}`}
            target="_blank"
          ...
          <div className="flex items-center justify-center gap-2">
            <span className="inline-flex items-center rounded-full bg-secondary/50 px-3 py-1 text-xs font-bold text-muted-foreground uppercase tracking-wider">
              {platform === "instagram" ? "Instagram" : "TikTok"} Profile
            </span>
              ...
              >
                <div className="space-y-2.5 text-left">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Data is collected from the <span className="font-semibold text-foreground">latest {postsAnalyzed || 9} posts</span> of this {isInstagram ? "Instagram" : "TikTok"} account.
                  </p>
                  <div className="p-2.5 rounded-lg bg-background border border-border/50">
                    ...
                    <p className="text-xs font-mono font-bold text-foreground leading-relaxed">{formula}</p>
                  </div>
                  <p className="text-[11px] text-muted-foreground/60 leading-relaxed">
                    {isInstagram
                      ? "Based on the standard Instagram engagement rate calculation method used by HypeAuditor."
